...
Delegate configuration file generation to the `ami-config-writer` sub-agent. Do not write `.properties` files directly. Use the Agent tool:
...
Delegate schema file generation to the `ami-sql-builder` sub-agent. Do not write `.amisql` files directly. Use the Agent tool:
...
For each layout in the plan, delegate to the `ami-layout-architect` sub-agent. Use the Agent tool:
