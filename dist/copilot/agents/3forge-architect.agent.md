...
Delegate configuration file generation to the `3forge-config-writer` sub-agent. Do not write `.properties` files directly. Delegate to the named Copilot agent:
...
Delegate schema file generation to the `3forge-sql-builder` sub-agent. Do not write `.amisql` files directly. Delegate to the named Copilot agent:
...
For each layout in the plan, delegate to the `3forge-layout-architect` sub-agent. Delegate to the named Copilot agent:
