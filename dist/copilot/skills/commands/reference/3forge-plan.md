...
3. **Query the live instance for patterns** — Do NOT spawn a subagent:
   - **First**, if MCP tools have not been used yet in this conversation, use tool discovery to find/load the `3forge-runtime` `aidoc_getDocumentation`, `aidoc_search_patterns`, `aidoc_findMethodByName`, `aidoc_findMethodByDesc`, `aidoc_listMethodsInClass`, and `web_console` tool schemas before calling them.
