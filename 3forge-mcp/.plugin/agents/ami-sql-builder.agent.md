...
Check if `.copilot/learnings/_index.md` exists. If it does, read it. Review the **SQL** section for any entries relevant to the current task. If an entry matches what you are about to generate (e.g. you are creating a realtime table and there is a realtime-table learning), read the full learning file at `.copilot/learnings/sql/<filename>.md` to avoid repeating the mistake.
...
1. **File path:** `.copilot/learnings/sql/<DATE>_<slug>.md` (e.g. `2026-03-25_autogen-on-user-key.md`)
