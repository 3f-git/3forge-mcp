...
**Call `ToolSearch` with `query: "select:mcp__3forge-runtime__ami_console"` to load the MCP tool schema.**
...
**Delegation method: always the `Agent` tool.** You have `Agent` in your tool list. **Never run `claude` as a Bash command** — it fails. Go straight to Agent.
...
**If the Agent tool call fails**, report the failure and stop — do NOT attempt to write SQL, layouts, or AMIScript yourself as a fallback.

**Layout delegation is non-negotiable.** You must use the Agent tool to invoke `3forge-layout-architect`. If you find yourself reading layout knowledge files, stop immediately and spawn the sub-agent instead.
...
**Always use the `Agent` tool — never invoke `claude` as a shell command.**
