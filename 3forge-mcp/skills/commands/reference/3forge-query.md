...
3. **Query the live instance for syntax confirmation** — Do NOT spawn a subagent:
   - **First**, if MCP tools have not been used yet in this conversation, use ToolSearch to find/load the `3forge-runtime` `aidoc_getDocumentation` tool schema before calling it.
