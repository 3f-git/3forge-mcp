...
3. **Verify method signatures via the live instance** — Do NOT spawn a subagent:
   - **First**, if MCP tools have not been used yet in this conversation, use ToolSearch to find/load the `3forge-runtime` `aidoc_findMethodByName`, `aidoc_findMethodByDesc`, `aidoc_listMethodsInClass`, and `web_console` tool schemas before calling them.
